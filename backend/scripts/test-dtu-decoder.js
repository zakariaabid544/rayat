...
const { parseGatewaySignalUpdate, parseSensorUpdate } = require('../utils/sensor-update-parser');
...
function testMqttPingPayloadParsing() {
  const parsed = parseGatewaySignalUpdate({
  ...
  assert.equal(parsed.event, 'heartbeat');
}
